...
stack(
  ...
  s("Kicks*4").gain(0.9),
  s("~ Claps ~ Claps").gain(0.7),

  // Hats — enter at bar 2
  s("ClosedHats*8").gain(0.5).mask("<0 1 1 1>"),

  // Bass — enter at bar 2
  note("c2 ~ c2 eb2").s("Bass").lpf(400).release(0.1).mask("<0 1 1 1>").gain(0.8),

  // Melody — only bar 3 (hook)
  note("c4 eb4 g4 bb4").s("Synth").mask("<0 0 1 0>").gain(0.5),

  // Pad — bars 3 and 4
  chord("<Cm7 Fm7>").s("Chords").lpf(800).release(1).room(0.6).voicing().mask("<0 0 1 1>").gain(0.3)
)
```
...
```js
// Light swing
s("ClosedHats*8").late("0 .03 0 .03")

// Heavy swing (hip hop)
s("ClosedHats*8").late("0 .06 0 .06")

// Triplet-feel swing
s("ClosedHats*8").late("0 .04 0 .04")
```
...
s("ClosedHats*8").gain("[.4 .7 .5 .8]*2")

// Ghost notes on snare
s("~ [Snares:1 Snares:1] ~ Snares").gain("[~ [.3 .3] ~ .9]")

// Rising velocity
s("ClosedHats*8").gain("[.3 .4 .5 .6 .7 .8 .9 1]")
```
...
s("ClosedHats*8").lpf(sine.range(200, 8000).slow(8))

// Dramatic one-direction sweep over 16 cycles
note("c3*4").s("Bass").lpf(saw.range(100, 5000).slow(16))
```

### HPF Sweep (Tension/Release)
```js
// Filter out low end, then bring it back
s("Kicks*4").hpf(sine.range(20, 500).slow(4))
```

### Resonant Acid Sweep
```js
note("c2 c2 c3 c2").s("Bass")
...
```js
stack(
  s("Kicks*4"),                    // kick layer
  s("~ Claps ~ Claps"),           // clap layer
  s("ClosedHats*8"),              // hat layer
  note("c2 e2").s("Bass").lpf(400).release(0.1)  // bass layer
)
```
...
```js
cat(
  s("Kicks Snares Kicks Snares"),                // section A
  s("Kicks ClosedHats Snares [ClosedHats ClosedHats]") // section B
)
```

### Layered Sounds (one instrument)
```js
// Fat kick: two different kick variants layered
stack(
  s("Kicks:0").gain(0.8),
  s("eot:5").gain(0.5).lpf(100)
)
```
...
n("0 2 4 6 3 5 7 4").scale("C:minor").s("Stabs")

// Random notes from scale
n("[0 1 2 3 4 5 6 7]?").scale("C:minor").s("Synth").release(0.5)
```
...
```js
chord("<C^7 Dm7 G7 C^7>").s("Stabs").voicing()
```
...
s("Kicks Snares Kicks Snares").every(4, x => x.fast(2))

// Reverse every 3rd cycle
s("ClosedHats*8").every(3, x => x.rev())
```

### `.sometimes(fn)` — 50% chance each cycle
```js
s("Kicks Snares Kicks Snares").sometimes(x => x.speed(1.5))
```

### `.off(time, fn)` — Overlay a delayed, modified copy
```js
// Echo with pitch shift
note("c4 e4 g4 b4").s("Stabs")
...
s("ClosedHats*8").mask("<0 0 1 1>")

// Play first 2 of every 4 cycles
note("c4 e4").s("Synth").mask("<1 1 0 0>")
```
...
stack(
  s("Kicks*4"),                                    // always on
  s("ClosedHats*8").mask("<0 1 1 1>"),             // enters cycle 2
  s("~ Claps ~ Claps").mask("<0 0 1 1>"),          // enters cycle 3
  note("c2 e2").s("Bass").lpf(400).release(0.1).mask("<0 0 0 1>")  // enters cycle 4
)
```
...
```js
chord("<Dm7 G7 C^7 Am7>").s("Stabs").voicing()
```
